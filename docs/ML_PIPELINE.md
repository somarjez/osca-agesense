...
|---|---|---|---|
| `scaler.pkl` | MinMaxScaler (30-feature) | Raw 30-feature vector | Scaled feature vector [0,1] per feature |
| `cluster_assignment_knn_k5.pkl` | KNeighborsClassifier (k=5, euclidean) | Scaled 30-feature vector | Named cluster ID (1, 2, 3, or 4) directly |
| `umap_nd.pkl` | UMAP reducer (nn=10, euclidean) | Scaled features | 10-dimensional embedding (training only; fallback path at inference) |
| `kmeans.pkl` | KMeans (K=4, n_init=100) | 10-D UMAP embedding | Raw cluster ID (0, 1, 2, 3) (fallback path) |
...
| `cluster_assignment_knn_k5.pkl` | **Primary cluster assignment model** — KNN k=5 euclidean, MinMaxScaler·30-feature; 5-fold CV accuracy 0.9333. Predicts named IDs 1–4 directly. Generated by `osca5.ipynb`. |
| `cluster_assignment_metadata.json` | KNN artifact metadata: k, metric, n_features, feature_names, cv_accuracy, official clustering metrics. |
| `cluster_centroids_scaled.json` | **Fallback** — Cluster centroids in 30D scaled space, used for deterministic nearest-centroid assignment when KNN is unavailable. Generated by `python/scripts/generate_cluster_centroids.py` from notebook ground truth. |
...
9. **Feature scaling** — MinMaxScaler (30-feature ablated set, `scaler.pkl`) applied to the cluster feature vector
...
**Training pipeline:** raw features → `scaler.pkl` (MinMaxScaler, 30-feature ablated set) → `umap_nd.pkl` (30D → 10D UMAP, nn=10, euclidean) → `kmeans.fit()` in 10D UMAP space → 4 clusters → KNN trained on 30D scaled space
...
### Inference (live system — v2.0.0, deterministic)

**As of model version 2.0.0**, the live inference service uses a **KNN classifier (k=5, euclidean, MinMaxScaler·30-feature)** (`cluster_assignment_knn_k5.pkl`) as the primary cluster assignment method. The KNN predicts named IDs 1–4 **directly** — no post-hoc remapping required.

**Inference pipeline:** raw features → `scaler.pkl` (MinMaxScaler, 30 features) → 30D scaled vector → `cluster_assignment_knn_k5.pkl` → named ID (1, 2, 3, or 4)

`cluster_assignment_metadata.json` records the KNN artifact provenance: k=5, metric=euclidean, scaler=MinMaxScaler, n_features=30, feature_names, cv_accuracy=0.9333, official clustering metrics (Silhouette 0.5577, Davies-Bouldin 0.6492, Calinski-Harabász 6048.7).

`cluster_centroids_scaled.json` (30D scaled-space centroids) is the fallback when the KNN artifact is unavailable.

**Why KNN instead of UMAP+KMeans:**
- UMAP's `.transform()` is non-deterministic across CPU families and OS versions; enabling it live produced 2.1% match (broken)
...
Step 2: Cluster assignment
        v1.1.1+ with ENABLE_DETERMINISTIC_CLUSTER=true:
          KNN (k=5) in 30D MinMaxScaler scaled space from cluster_assignment_knn_k5.pkl
...
As of model version 2.0.0, the inference service uses a **KNN classifier (k=5, euclidean, MinMaxScaler·30-feature)** (`cluster_assignment_knn_k5.pkl`) for cluster assignment. The KNN predicts named IDs 1–4 directly. `cluster_centroids_scaled.json` (30D scaled-space centroids) is the fallback.

Because every device loads the same committed `cluster_assignment_knn_k5.pkl`, the KNN prediction is bit-for-bit identical regardless of CPU, OS, or Python version. UMAP is not called at inference time.
