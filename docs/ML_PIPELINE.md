...
**Model:** KMeans with K=3, trained in `osca5.ipynb` on 283 Pagsanjan seniors.

**Training pipeline:** raw features → `scaler.pkl` → `umap_nd.pkl` (31D → 10D UMAP) → `kmeans.fit()` in 10D UMAP space → 3 clusters

The notebook assigned each senior to one of 3 clusters in 2D UMAP space using `KMeans.fit_predict()`. These assignments are the ground truth stored in `senior_predictions.csv`.
...
Expected output: `[OK] Centroids written: 3 clusters from 283 seniors`
