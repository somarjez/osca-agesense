...
| `cluster_mapping.json` | Raw KMeans ID → named cluster ID | `{"0": 1, "1": 2, "2": 3, "3": 4}` exactly |
...
| `cluster_assignment_knn_k5.pkl` | **Primary** — KNN classifier k=5 euclidean, MinMaxScaler·30-feature; predicts named IDs 1–4 directly; CV accuracy 0.9333 | `cluster_assignment_metadata.json` |
| `cluster_assignment_metadata.json` | KNN artifact metadata: k, metric, n_features, cv_accuracy, official metrics | — |
| `cluster_centroids_scaled.json` | **Fallback** — cluster centroids in 30D scaled space (nearest-centroid assignment) | 4 centroids × 30 features |
| `predictions/senior_predictions.csv` | Notebook results for 360 seed seniors | 360 rows (gitignored) |

**Critical notes:**
- `feature_list.json` must contain **30 features** (the ablated MinMaxScaler set). `scaler.pkl` is MinMaxScaler, n_features_in_=30.
- `cluster_mapping.json` is `{"0": 2, "1": 4, "2": 3, "3": 1}` — the raw KMeans ID → named ID permutation discovered during training. The **KNN directly predicts named IDs 1–4** and does not use cluster_mapping.json at inference time (it is stored for reference/training reproducibility only).
- `cluster_assignment_knn_k5.pkl` is the **primary live cluster assignment model** (v2.0.0). Fallback: `cluster_centroids_scaled.json` (nearest-centroid in 30D). Generated by `osca5.ipynb`. `cluster_centroids_scaled.json` is generated by `python\scripts\generate_cluster_centroids.py`.
...
**Notebook analytical distribution for 360 seniors:**
- HIGH: 75  ·  CRITICAL: 2 (≥0.70; shown as HIGH+urgent in live 3-level display)  ·  MODERATE: 233  ·  LOW: 50
- C1: 69  ·  C2: 94  ·  C3: 91  ·  C4: 106

### Path B — Live model (`prediction_source = live_model`) — canonical

**Used for:** New seniors added after seeding (always), and **all** seniors in this deployment (`ENABLE_NOTEBOOK_OVERRIDES=false`). Live 3-level display distribution (360): HIGH=77 (including 2 with `priority_flag='urgent'`), MODERATE=233, LOW=50. KNN cluster assignment (CV accuracy 0.9333) gives ~87% agreement with notebook UMAP+KMeans labels; risk scores match notebook to within 0.02 for 99.7% of seniors. See [model-validation-defensible-statements.md](model-validation-defensible-statements.md).

Every senior is scored live using the following pipeline:

1. **Preprocess** — raw profile → feature vector (30 ablated features) → MinMaxScaler (`scaler.pkl`) → scaled vector
2. **Cluster assignment (deterministic, v2.0.0)** — **KNN (k=5, euclidean)** from `cluster_assignment_knn_k5.pkl`; predicts named ID 1–4 directly. Fallback: nearest centroid in 30D scaled space using `cluster_centroids_scaled.json`. UMAP and KMeans are **not called** for cluster assignment on individual seniors.
...
- `cluster_mapping.json` maps `{0→1, 1→2, 2→3, 3→4}` correctly
...
Transfer `osca_models_v2.0.0.zip` to the target device via USB or shared folder.
