...
| `cluster_centroids_scaled.json` | Cluster centroids in 31D scaled space (nearest-centroid assignment) | 4 centroids × 31 features |
...
- `cluster_mapping.json` must be exactly `{"0": 1, "1": 2, "2": 3, "3": 4}`. This maps raw KMeans cluster IDs to named cluster IDs: raw 0 → C1 (High Functioning), raw 1 → C2 (Stable Ageing), raw 2 → C3 (Environmentally/Financially Vulnerable), raw 3 → C4 (Low Functioning / Multi-Domain Priority).
- `cluster_centroids_scaled.json` is the source of truth for **live cluster assignment** (deterministic clustering, introduced in v1.1.1 and retained in the v2.0.0 K=4 build). The inference service computes the nearest of the 4 centroids in 31D scaled space instead of calling UMAP+KMeans. This is deterministic across all CPU/OS combinations. Generated by `python\scripts\generate_cluster_centroids.py`.
...
2. **Cluster assignment (deterministic, v1.1.1+; K=4 in v2.0.0)** — nearest centroid in 31D scaled space using `cluster_centroids_scaled.json`. The Euclidean distance from the scaled vector to each of the 4 stored centroids is computed; the closest centroid wins. UMAP and KMeans are **not called** for cluster assignment on individual seniors.
