...
| **Clustering pipeline** | MinMaxScaler · 30-feature ablated set · UMAP nn=10 euclidean · KMeans n_init=100 | `cluster_assignment_metadata.json` |
| Silhouette score (K=4, 30-feature MinMaxScaler) | **0.5577** | `clustering_evaluation.csv` |
| Davies-Bouldin index (K=4) | **0.6492** | `clustering_evaluation.csv` |
| Calinski-Harabasz index (K=4) | **6048.7** | `clustering_evaluation.csv` |
| **KNN cluster assignment** | k=5, euclidean; 5-fold stratified CV accuracy **0.9333**; predicts named IDs 1–4 directly | `cluster_assignment_knn_k5.pkl` |
| **Risk-level concordance accuracy** | **0.5472** (rule↔ensemble; y_true=rule label — *not* ground-truth; see note) | `risk_level_validation_summary.json` |
| Risk-level weighted F1 | **0.5505** | `risk_level_validation_summary.json` |
| HIGH recall (ensemble correctly flags HIGH true-labels) | **0.8947** | `risk_level_validation_summary.json` |
| Priority capture rate (HIGH/CRITICAL true → flagged HIGH/CRITICAL) | **1.0** — all 19 high-priority cases correctly flagged | `risk_level_validation_summary.json` |
| False escalation rate (LOW/MODERATE true → predicted HIGH/CRITICAL) | **0.1701** (58 of 341 low-priority seniors over-escalated) | `risk_level_validation_summary.json` |
...
**Note on the HIGH-risk urgent sub-tier and CRITICAL analytical band:** Within the HIGH-risk seniors, those with composite ≥ 0.70 receive `priority_flag='urgent'`. The notebook/reports use a 4-level **analytical** scheme (LOW/MODERATE/HIGH/CRITICAL ≥0.70 = urgent-review flag, *not a clinical diagnosis*); the **live app displays 3 levels** (LOW/MODERATE/HIGH) and surfaces the ≥0.70 cases via `priority_flag='urgent'` (orange ring + warning icon). The notebook emits CRITICAL for 2 seniors — composite ≥0.747; the live system classifies both as HIGH + urgent, accounting for the 2-senior gap between notebook HIGH=75/CRITICAL=2 and live HIGH=77.

**Note on risk-level concordance accuracy (0.5472):** The accuracy metric compares *rule-based labels* (y_true) against *ensemble labels* (y_pred); it is **not** a ground-truth classification accuracy — no externally validated outcome labels exist. The 0.5472 figure reflects MODERATE/HIGH boundary disagreement between the rule engine and the GBR/RFR ensemble. `priority_capture_rate = 1.0` confirms all 19 rule-HIGH seniors (the high-priority population) were predicted HIGH or CRITICAL by the ensemble — none were missed. `false_escalation_rate = 0.1701` indicates 58 rule-LOW/MODERATE seniors were escalated by the ensemble; these are MODERATE/HIGH borderline cases for case-management review, not a critical failure.

**Note on cluster quality across K:** The notebook's `clustering_evaluation.csv` reports, for the **30-feature MinMaxScaler** ablated set (360-senior retrain): K=2 (Silhouette 0.8726), K=3 (0.5903), **K=4 (0.5577)**, K=5 (0.4944), K=6 (0.4679). K=4 was adopted to give a distinct "Environmentally & Financially Vulnerable" group (C3) separate from "Low Functioning / Multi-Domain Priority" (C4) — a care-actionable distinction for OSCA service delivery. K=2 has a higher silhouette but merges all four profiles into two undifferentiated groups; K=3 merges C3 and C4. K=4 retains strong cluster quality (Silhouette 0.5577, Davies-Bouldin 0.6492, Calinski-Harabasz 6048.7) while providing maximum care-actionable granularity.
...
- **Cluster coherence** is monotonic: mean composite risk rises 0.288 → 0.386 → 0.401 → 0.544 across clusters 1→4, confirming the clusters are meaningfully ordered by need. Cluster assignment uses **KNN (k=5, euclidean, MinMaxScaler·30-feature)** with 5-fold CV accuracy 0.9333; the KNN predicts named IDs 1–4 directly without post-hoc remapping.
...
The **four-cluster** structure segments the population into care-actionable profiles: C1 High Functioning / Well-Supported, C2 Stable Ageing / Moderate Support, C3 Environmentally & Financially Vulnerable, and C4 Low Functioning / Multi-Domain Priority. K=4 was chosen from the notebook's K-sweep (`clustering_evaluation.csv`, 30-feature MinMaxScaler ablated set): it retains strong cluster quality (Silhouette **0.5577**, Davies-Bouldin **0.6492**, Calinski-Harabasz **6048.7**) while separating the environmentally/financially vulnerable seniors (C3) from the multi-domain low-functioning seniors (C4) — a distinction that maps to different OSCA interventions (livelihood/housing support vs comprehensive case management). Cluster coherence is confirmed by monotonically increasing mean composite risk across C1→C4 (0.288 → 0.386 → 0.401 → 0.544).
...
*Technical:* The notebook clusters with UMAP+KMeans, which is non-reproducible per record (single-point UMAP `transform()` varies by device/version — enabling it live gave 2.1%). The live system uses **KNN (k=5, euclidean, MinMaxScaler·30-feature; CV accuracy 0.9333)** which is deterministic and device-independent. The 47 differing seniors are boundary cases equidistant between clusters. None of the cluster-label mismatches differ in risk level; their care plans are identical. 100% agreement with the notebook's UMAP+KMeans is unreachable deterministically because the reference method itself is non-deterministic.
...
*Technical:* Part of the composite comes from a rule-based engine with no trainable parameters (cannot overfit); the learned GBR/RFR portion is validated out-of-sample, reproducing risk to 99.7–100%. Cluster quality metrics (Silhouette **0.5577**, Davies-Bouldin **0.6492**, Calinski-Harabász **6048.7**; 30-feature MinMaxScaler ablated set) confirm genuine population structure. The acknowledged limitation is the absence of an independent holdout set.
...
*Technical:* The notebook K-sweep (`clustering_evaluation.csv`, 30-feature MinMaxScaler ablated set, 360-senior retrain) gives Silhouette K=2 0.8726, K=3 0.5903, **K=4 0.5577**, K=5 0.4944, K=6 0.4679. K=2 has the highest silhouette but merges all four care profiles into two undifferentiated groups (no separation between financially vulnerable and low-functioning seniors). K=4 separates the **Environmentally & Financially Vulnerable** group (C3) from the **Low Functioning / Multi-Domain Priority** group (C4), which require different OSCA interventions (livelihood/housing vs comprehensive case management). K=3 merged these into one group, losing that care distinction; K=5+ creates splits with no distinct OSCA service-tier mapping.
...
*Technical:* Cluster assignment and risk scoring use different model components. Cluster reflects the overall 30-feature MinMaxScaler functional profile; risk is the domain-weighted GBR/RFR + rule ensemble (medical 28%, financial 18%, etc.). A C1 senior with strong function but a high-severity chronic condition can be pushed to MODERATE by the medical domain weight — correct detection of a specific elevated risk within an otherwise high-functioning profile.
