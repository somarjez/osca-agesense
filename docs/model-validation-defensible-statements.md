...
| Cluster match: live KNN vs notebook labels | **313 / 360 = 86.9%** | `validate_system.py` |
...
| — LOW risk | 50 seniors (13.9%) | `validate_system.py` |
| — MODERATE risk | 233 seniors (64.7%) | `validate_system.py` |
| — HIGH risk | 77 seniors (21.4%) | `validate_system.py` |
| — HIGH risk, urgent flag (composite ≥ 0.70) | 2 seniors (shown as HIGH + Urgent in dashboard) | `inference_service.py` |
| **Cluster distribution — notebook (training labels, 360)** | C1=69, C2=94, C3=91, C4=106 | `senior_predictions.csv` |
| **Cluster distribution — live KNN (360 seniors)** | C1=69, C2=94, C3=91, C4=106 (approx; run validate_system.py for exact) | `validate_system.py` |
...
- **Cluster assignment** (KNN k=5 classifier vs notebook UMAP+KMeans labels) agreed for **313 of 360 (86.9%)** — the deterministic ceiling, explained in Part 2. KNN 5-fold CV accuracy: 0.9333.
...
3. **Cluster boundary uncertainty for 13.1% of seniors.** 47 seniors sit near a cluster boundary and receive a different label under the live KNN (30-D) vs notebook (UMAP+KMeans) geometry; their assignment should be treated as approximate. Their risk level and recommendations are unaffected.
...
*Technical:* The notebook clusters with UMAP+KMeans, which is non-reproducible per record (single-point UMAP `transform()` varies by device/version — enabling it live gave 2.1%). The live system uses a trained **KNN k=5 classifier** (CV accuracy 0.9333) in 30-D scaled space — bit-for-bit identical on every device. The 47 differing seniors (13.1%) are boundary cases: nearest-vs-second-nearest centroid margin 0.1002 vs 0.3327 for matches (3.3× tighter). None differ in risk level; their care plans are identical. 100% is unreachable deterministically because the reference method itself is non-deterministic.
*Plain:* "87% agreement is strong, and the KNN model was validated at 93.3% accuracy. The 13% who differ are on-the-fence seniors — their care plan is the same either way, and our method gives the same answer on every computer."
...
*Document version: 2.1.0 | System: AgeSense OSCA v2.0.0 (K=4, N=360) | Updated: 2026-07-01 | Prediction mode: ENABLE_NOTEBOOK_OVERRIDES=false (live model default)*
