...
function helpCentre() {
    ...
    const items = [
        ...
        // Senior Records
        { topic: 'senior-records', q: 'How do I add a new senior citizen?', a: 'Go to Senior Records → New Profile in the sidebar. Fill in all required fields — name, date of birth, barangay — and submit. A System ID is assigned automatically. If the senior already has an official OSCA ID on hand, enter it in the OSCA ID field; otherwise leave it blank and add it later once assigned.' },
        ...
        { topic: 'senior-records', q: 'What is the difference between System ID and OSCA ID?', a: 'System ID is generated automatically when a profile is created and is used internally to identify the record — it never changes. OSCA ID is the official identification number issued by your local Office for Senior Citizens Affairs; it is entered by staff once known and shows as "Unassigned" until then. Both must be unique across the system — the system will reject a duplicate OSCA ID when saving a profile.' },
        ...
        { topic: 'senior-records', q: 'How does bulk upload work?', a: 'Go to Senior Records and click Bulk Upload. Download the CSV template, fill in the required columns (first_name, last_name, barangay, dob, gender), then upload the file. The optional osca_id column lets you supply each senior\'s official OSCA ID — leave it blank if not yet known, and it will show as "Unassigned". A duplicate OSCA ID is imported without it (with a warning) rather than blocking the row. Rows with missing required fields are skipped. The system automatically runs assessments for all imported seniors within a minute.' },
        ...
        { topic: 'general', q: 'Can two seniors share the same OSCA ID or System ID?', a: 'No. Both identifiers must be unique across the entire system. The system will reject a duplicate System ID or OSCA ID when saving a profile and display an error message. An OSCA ID left blank ("Unassigned") does not count as a duplicate.' },
